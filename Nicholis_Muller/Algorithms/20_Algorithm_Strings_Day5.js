...
indexString([1, 3, 4, 5, 7, 8, 10, 13, 14, 15, 19, 23]);
...
function comSuf(arr) {
    ...
    while (counter >= 0) {
        for (var i = 0; i < arr.length; i++) {
            var last = arr[i].length + counter2;

            if (str[str.length - 1] !== arr[i][last]) {
                str.push(arr[i][last]);
            }
        }
        counter2 -= 1;
    ...
    var final = str.reverse().join("");
    console.log(final)
}

comSuf(["deforestation", "citation", "conviction", "incarceration"])
